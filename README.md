...
当前版本会在首次正常启动后自动注册为当前用户的开机自启动项，因此 Windows 登录后 Peeklet 会自动在后台托盘中运行。

程序启动后会自动向当前用户注册标准卸载信息，因此你可以在 Windows 的“应用和功能”中找到 Peeklet 并执行卸载。

卸载时会调用同目录下的 Peeklet.Uninstaller.exe，移除当前用户的开机自启动项，并删除 Peeklet 所在目录中的程序文件。